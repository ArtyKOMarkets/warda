...
const toolPath = (script) => BUNDLED
    ? fileURLToPath(new URL("tools/" + script.slice(script.lastIndexOf("/") + 1).replace(/\.ts$/, ".js"), import.meta.url))
    : repo(script);
...
const sompi = (v, what) => {
    ...
    if (s.endsWith("sompi")) {
        ...
        if (!/^\d+$/.test(n))
            die(`${what}: "${v}" is not a whole number of sompi.`);
        return n;
    }
    try {
...
const agentSecret = (cfg) => {
    if (process.env.WARDA_SK)
        return process.env.WARDA_SK.trim();
...
switch (verb) {
    ...
    case "--version":
        ...
        console.log(JSON.parse(readFileSync(fileURLToPath(new URL(BUNDLED ? "../package.json" : "package.json", import.meta.url)), "utf8")).version);
        break;
    case "node": {
    ...
    case "key": {
        const out = flag("out");
        process.exit(run("sdk/tools/new-key.ts", [
            "--label",
            flag("label", "wallet"),
            ...(out ? ["--out", out] : []),
    ...
    case "wallet": {
        ...
        process.exit(run(sub === "consolidate" ? "sdk/tools/consolidate.ts" : "sdk/tools/wallet.ts", [
            ...(key ? ["--key", key] : []),
            ...rpcArgs(cfg),
            ...(has("submit") ? ["--submit"] : []),
            ...(has("force") ? ["--force"] : []),
    ...
    case "fund": {
        ...
        if (!payees) {
            die("warda fund --payees <file> --budget <KAS> --rate <price of one KAS>\n\n" +
                "  Same allowlist rules as `warda grant`: one address per line, fixed at\n" +
                "  creation, and it IS the authority rather than a config.", 2);
        }
        if (!existsSync(payees))
            die(`--payees: no such file: ${payees}`, 2);
        const cfg = readConfig();
        const funder = flag("key");
        const funderEnv = funder
    ...
    case "grant": {
        ...
        if (!payees) {
            die("warda grant --payees <file>\n\n" +
                ...
                "  the proof a spend needs — lose it and the grant can be revoked, never spent.", 2);
        }
        if (!existsSync(payees))
            die(`--payees: no such file: ${payees}`, 2);
    ...
    case "balance": {
        ...
        if (viaBorsh) {
            ...
            try {
                ...
                console.error(`(no node: ${e.message})\n`);
            }
        }
        const B = (k) => BigInt(m[k] ?? 0);
        ...
        for (const p of payees.slice(0, 6))
            console.log(`  ${" ".repeat(16)}${p}`);
        if (daa !== null) {
            ...
            if (daa < notBefore)
                line("not before", `${notBefore - daa} blocks to go. Until then every spend is refused.`);
        }
    ...
    case "pay": {
        ...
        if (!url)
            die("warda pay <url>   — the endpoint that answers 402.", 2);
        const cfg = readConfig();
        const grant = flag("grant") ?? cfg.grant;
        ...
        process.exit(run("agents/tools/buy.ts", [
            url,
            "--id",
    ...
    case "activity": {
        ...
        if (!existsSync(dir))
            die(`nothing yet — ${dir} does not exist. \`warda pay <url>\` writes there.`);
        const files = readdirSync(dir).filter((f) => f.endsWith(".json")).sort();
        if (!files.length)
            die(`nothing yet in ${dir}.`);
        console.log();
        for (const f of files.slice(-Number(flag("n", "20")))) {
            ...
            console.log(`  ${mark} ${String(r.at ?? "").slice(0, 19).replace("T", " ")}  ${amt.padStart(10)}  ` +
    ...
    case "find": {
        ...
        if (!grant)
            die("warda find <grant.json>   — or run it where `warda grant` remembered one.");
        const vendor = flag("vendor") ?? (cfg.payees && existsSync(cfg.payees)
            ? readFileSync(cfg.payees, "utf8").split(/\r?\n/).map((l) => l.replace(/#.*$/, "").trim()).filter(Boolean)[0]
            : undefined);
        if (!vendor)
            die("warda find needs --vendor <address>: a grant is found again through a transaction that spent it.");
        process.exit(run("sdk/tools/follow-grant.ts", [
            grant,
            "--vendor",
            vendor,
            ...rpcArgs(cfg),
    ...
    case "reclaim": {
        ...
        const grant = rest.find((a) => a.endsWith(".json")) ?? cfg.grant;
        if (!grant) {
            die(`warda ${verb} <grant.json>   — or run it where \`warda grant\` remembered one.\n\n` +
                (verb === "revoke"
                    ...
                    : "  The scheduled end. Signed by the PRINCIPAL key, available only once the\n" +
                        "  chain has passed the grant's expiry."), 2);
        }
        ...
        const env = keyPath
            ...
            : {};
        process.exit(run("sdk/tools/build-exit.ts", [
            grant,
    ...
    case "fee": {
        ...
        const cfg = readConfig();
        process.exit(run("sdk/tools/measure-relay-fee.ts", [
            ...rpcArgs(cfg),
            ...(flag("fee") ? ["--fee", flag("fee")] : []),
            ...(flag("network") ? ["--network", flag("network")] : []),
        ], { WARDA_SK: agentSecret(cfg) }));
    ...
    case "mcp": {
        ...
        let bin;
        try {
            bin = fileURLToPath(new URL("dist/server.js", pathToFileURL(createRequire(import.meta.url).resolve("@warda_protocol/mcp/package.json"))));
        }
        catch {
            die("the MCP server is not installed.\n" +
                "  It ships separately, because it versions separately:\n" +
                "    npm install @warda_protocol/mcp", 2);
        }
