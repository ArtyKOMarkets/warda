          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[760px] text-left text-[13.5px]">
              <thead className="border-b border-line text-[12px] text-fg-3">
                <tr>{["Agent", "Status", "Spendable", "Budget used", "Per payment", "Ends in", "Last active"].map((h) => <th key={h} className="px-5 py-3 font-medium">{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-line">
                {list.map((a) => (
                  <tr key={a.key} onClick={() => go("agents", a.key)} className="cursor-pointer transition hover:bg-raised/50">
                    ...
                    <td className="px-5"><Kas value={kas(a.status === "ended" ? 0 : a.remaining)} /></td>
