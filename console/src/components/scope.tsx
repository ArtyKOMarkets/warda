...
export function ScopeSwitch({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("flex rounded-lg border border-line-strong bg-surface p-0.5", className)}>
      {([["mine", "Yours", "Yours"], ["warda", "Warda's agents", "Warda's"]] as const).map(([v, l, shortl]) => (
        <button key={v} onClick={() => setScope(v)} className={cn("h-8 whitespace-nowrap rounded-md px-2.5 text-[12.5px] font-medium transition md:h-7", scope === v ? "bg-raised text-fg" : "text-fg-3 hover:text-fg-2")}>
          <span className="md:hidden">{shortl}</span><span className="hidden md:inline">{l}</span>
        </button>
      ))}
    </div>
  );
