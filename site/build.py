...
# WalletConnect's relay needs a project id (Reown, cloud.reown.com). It is
# public — every dApp ships its own in page source — so it lives in a committed
# file rather than an environment variable a deploy could quietly lack. Empty
...
AGENT_IDS = [f.stem.split("-")[1] for f in sorted((here / "src").glob("agent-0[0-9][0-9].json"))]
...
COPIES = [
    ...
    "robots.txt",
    "sitemap.xml",
    # Fetched by /app only when somebody presses "Connect with WalletConnect".
    # Generated by ops/walletconnect/build.mjs from a lockfile.
    "walletconnect-browser.js",
    # Which ways into KAS exist and whether each leg works, dated. Read by
    # /app's Fund an agent; edited by hand when a status is checked.
    "routes.json",
]
...
GENERATED = {
    # Which agents have a published reading, derived from the readings that
    # exist. The console at /app had this list typed into its own script as
    # ["001" … "006"], which is the eleventh instance of extracted-then-copied
    # here and the same shape as the one that cost `first-contact` three
    # packages: a seventh agent would have had a page, a reading and a row in
    # the sitemap, and been invisible on the console for as long as it took
    # somebody to notice. Generated, so publishing a reading is the only thing
    # anybody has to remember.
    "agents.json": {
        "_comment": (
            "Generated by site/build.py from the readings that exist in src/. "
            "The list of agents with a published reading — not the list of agents "
            "that exist, which is not knowable from here."
        ),
        "ids": AGENT_IDS,
    },
...
# The covenant audit report, at /audit.
#
# Copied verbatim rather than wrapped: it is already a whole document, and it
# is generated by `cargo run --bin audit` in covenant/harness rather than
# written by hand. Publishing it is the only way somebody who is not in this
# repo can read it, and `ops/check-audit.mjs` refuses a copy older than the
# covenant it describes — a published audit of a covenant that has since
# changed is worse than no published audit.
_audit = here.parent / "covenant" / "AUDIT.html"
if _audit.exists():
    (_web / "audit.html").write_bytes(_audit.read_bytes())
    print(f"web/audit.html: {_audit.stat().st_size / 1024:>6.1f} KB from covenant/AUDIT.html")
else:
    print("web/audit.html: no report (run `cargo run --bin audit` in covenant/harness)")
