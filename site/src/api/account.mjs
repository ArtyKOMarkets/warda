...
// ops/api/node_modules/@neondatabase/serverless/index.mjs
...
// ops/api/node_modules/@noble/hashes/utils.js
function isBytes(a2) {
...
// ops/api/node_modules/@noble/hashes/_md.js
...
// ops/api/node_modules/@noble/hashes/sha2.js
...
// ops/api/node_modules/@noble/curves/utils.js
...
// ops/api/node_modules/@noble/curves/abstract/modular.js
...
// ops/api/node_modules/@noble/curves/abstract/curve.js
...
// ops/api/node_modules/@noble/hashes/hmac.js
var _HMAC = class {
  oHash;
  iHash;
  blockLen;
...
// ops/api/node_modules/@noble/curves/abstract/der.js
...
// ops/api/node_modules/@noble/curves/abstract/weierstrass.js
...
// ops/api/node_modules/@noble/curves/secp256k1.js
var secp256k1_CURVE = {
...
// ops/api/node_modules/@noble/hashes/_blake.js
...
// ops/api/node_modules/@noble/hashes/blake2.js
...
// ops/api/node_modules/@noble/hashes/sha3.js
var _0n7 = BigInt(0);
var _1n5 = BigInt(1);
var _2n4 = BigInt(2);
var _7n2 = BigInt(7);
...
var HEX64 = /^[0-9a-f]{64}$/;
var ADDR = /^kaspa(test)?:[a-z0-9]{61,63}$/;
var clean2 = (v2, max) => String(v2 == null ? "" : v2).replace(/[\u0000-\u001f\u007f]/g, " ").trim().slice(0, max);
...
function publicRequest(r) {
  return {
    ...
    payeesAsked: r.payees,
    note: r.status === "declined" ? r.note : void 0,
    grant: r.status === "issued" ? { address: r.grant_address, txid: r.txid, manifest: r.manifest, payees: r.grant_payees } : void 0
  };
}
async function grantRequests(req, res, q, op, url) {
  if (op === "request" && req.method === "GET") {
    const id = clean2(url.searchParams.get("id"), 32);
  ...
  if (op === "request" && req.method === "POST") {
    ...
    const agentKey = clean2(b2.agentKey, 80).toLowerCase();
    ...
    const payees = (Array.isArray(b2.payees) ? b2.payees : String(b2.payees || "").split(/[\s,]+/)).map((x2) => clean2(x2, 80).toLowerCase()).filter(Boolean);
    if (payees.length > 6) throw new Refuse(400, "payees", "At most six payees of your own.");
    for (const x2 of payees) if (!HEX64.test(x2) && !ADDR.test(x2)) throw new Refuse(400, "payees", x2.slice(0, 20) + "\u2026 is neither a kaspa address nor a 64-hex public key.");
    if (payees.includes(agentKey)) throw new Refuse(400, "payees", "The agent's own key cannot be one of its payees.");
    const project = clean2(b2.project, 120), contact = clean2(b2.contact, 120), about = clean2(b2.about, 600);
    ...
    await q(
      "insert into grant_requests (id, agent_key, payees, project, contact, about, ip_hash) values ($1,$2,$3,$4,$5,$6,$7)",
  ...
  if (op === "issue" && req.method === "POST") {
    const b2 = await body(req);
    const id = clean2(b2.id, 32);
    checkManifest(b2.manifest);
    const address = clean2(b2.address, 80), txid = clean2(b2.txid, 64).toLowerCase();
    ...
    const payees = Array.isArray(b2.payees) ? b2.payees.map((x2) => clean2(x2, 80)).slice(0, 16) : [];
    const rows = await q(
      "update grant_requests set status = 'issued', decided_at = now(), manifest = $2, grant_address = $3, txid = $4, grant_payees = $5 where id = $1 returning *",
      [id, JSON.stringify(b2.manifest), address, txid || null, JSON.stringify(payees)]
    );
  ...
  if (op === "decline" && req.method === "POST") {
    const b2 = await body(req);
    const rows = await q(
      "update grant_requests set status = 'declined', decided_at = now(), note = $2 where id = $1 returning *",
      [clean2(b2.id, 32), clean2(b2.note, 300) || null]
    );
    if (!rows.length) throw new Refuse(404, "request", "No request with that id.");
    return send(res, 200, { ok: true, request: publicRequest(rows[0]) });
  }
