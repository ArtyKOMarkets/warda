...
function renderWallet(d) {
  ...
  if (retired || !a.onChain) {
    ...
    /* A weekly agent gets a new grant every Monday: its grant ending is the
       week being done, not the agent being over. */
    var wk = d.weekly;
    bal.textContent = retired ? (wk ? "week done" : "ended") : "—";
    bal.className = "wal-amt ended";
    set("w-balsub", retired ? (wk ? wk.label + " · next grant " + (wk.schedule || "next week") : "the grant holds nothing") : "no reading");
  } else {
  ...
  if (a.remaining && a.budget) {
    row(mark("✓"), retired ? (d.weekly ? "returned" : "unspent") : "budget left", a.remaining + " of " + a.budget);
  }
  ...
  set(
    ...
    act.payments
      ...
      : d.weekly && d.weekly.delegatedTo
        ? "through Scout · " + d.weekly.delegatedTo.payments + " payments · " + d.weekly.delegatedTo.spent
        : "nothing yet",
  );
  ...
  if (items.length === 0) {
    ...
    var dt = d.weekly && d.weekly.delegatedTo;
    if (dt) {
      /* The orchestrator pays nobody itself: it hands Scout a piece of the
         budget, and Scout's payments are charged home to it at settlement. */
      var span = document.createElement("span");
      span.style.gridColumn = "1 / -1";
      var link = document.createElement("a");
      link.setAttribute("href", ["", "agent-" + dt.id].join("/"));
      link.textContent = "Scout, agent #" + dt.id;
      span.appendChild(document.createTextNode("Its payments this week were made by "));
      span.appendChild(link);
      span.appendChild(document.createTextNode(" out of the piece it delegated: " + dt.payments + " record" +
        (dt.payments === 1 ? "" : "s") + ", " + dt.spent + ", charged home here at settlement."));
      none.appendChild(span);
    } else none.textContent = "No payment has been made from this grant.";
    list.appendChild(none);
  } else {
    items.slice(0, 8).forEach(function (it) {
      var li = document.createElement("li");
      var s1 = document.createElement("span"); s1.className = "a"; s1.textContent = it.amount;
      /* The growth fleet's purchases name the projects it researched — people
         it may write to. The payment is public on chain; the list of who is
         being looked at does not need to be on a web page. */
      var who = d.weekly ? String(it.who).replace(/\?.*$/, "") + " · one project record" : it.who;
      var s2 = document.createElement("span"); s2.className = "w"; s2.textContent = who;
